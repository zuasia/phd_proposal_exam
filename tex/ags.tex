...
\subfloat[Control loop overview.] {
   \includegraphics[trim=0 250 0 260,clip,width=.625\linewidth]{graphs/control-loop.pdf}
   \label{fig:control-loop} 
}
\subfloat[CPM behavior.] {
   \includegraphics[trim=0 20 0 20,clip,width=.385\linewidth]{graphs/CPM-structure.pdf}
...
\subsection{Efficiency Analysis of Active Timing Margin on Multicore}
...
\begin{figure}[t]
    ...
    \subfloat[Power saving.] {
      \includegraphics[trim=0 0 0 0,clip,width=.4\linewidth]{graphs/raytrace_pwr.pdf}
      \label{fig:raytrace-ineff_pwr} 
    }
    \subfloat[Energy reduction.] {
       \includegraphics[trim=0 0 0 0,clip,width=.4\linewidth]{graphs/raytrace_edp.pdf}
...
\centering
    \subfloat[Frequency-boosting mode.] {
        \includegraphics[trim=0 45 65 35,clip,width=.38\linewidth]{graphs/lucb_freq.pdf}
        \label{fig:lucb-ineff_freq} 
      }
    \subfloat[Execution time.] {
        \includegraphics[trim=0 0 65 0,clip,width=.38\linewidth]{graphs/lucb_time.pdf}
...
\centering
  \subfloat[Power-saving mode.] {
    \includegraphics[trim=0 0 0 0,clip,width=.45\linewidth]{graphs/power_saving_variation.pdf}
    \label{fig:pwrvariation} 
  }
  \subfloat[Frequency-boosting mode.] {
    \includegraphics[trim=0 0 0 0,clip,width=.45\linewidth]{graphs/freq_saving_variation.pdf}
...
\begin{figure*}[t]
     \subfloat[raytrace.] {
        \includegraphics[trim=0 0 0 0,clip,width=.2\linewidth]{graphs/current_raytrace.pdf}
        \label{fig:raytrace_ir} 
      }
      \hspace*{-.1in}
      \subfloat[barnes.] {
        \includegraphics[trim=0 0 0 0,clip,width=.2\linewidth]{graphs/current_barnes.pdf}
        \label{fig:barnes_comp} 
      }
      \hspace*{-.1in}
      \subfloat[blackscholes.] {
        \includegraphics[trim=0 0 0 0,clip,width=.2\linewidth]{graphs/current_blackscholes.pdf}
        \label{fig:blackscholes_comp} 
      }
      \hspace*{-.1in}
      \subfloat[bodytrack.] {
        \includegraphics[trim=0 0 0 0,clip,width=.2\linewidth]{graphs/current_bodytrack.pdf}
        \label{fig:bodytrack_comp} 
      }
     \hspace*{-.1in}
      \subfloat[ferret.] {
        \includegraphics[trim=0 0 0 0,clip,width=.2\linewidth]{graphs/current_ferret.pdf}
        \label{fig:ferret_comp} 
      }
\vspace*{-.15in}
      \hspace*{-.05in} 
      \subfloat[lu\_ncb.] {
        \includegraphics[trim=0 0 0 0,clip,width=.2\linewidth]{graphs/current_lu_ncb.pdf}
        \label{fig:lu_ncb_comp} 
      }
      \hspace*{-.1in} 
      \subfloat[ocean\_cp.] {
        \includegraphics[trim=0 0 0 0,clip,width=.2\linewidth]{graphs/current_ocean_cp.pdf}
        \label{fig:ocean_cp_comp} 
      }
      \hspace*{-.1in} 
      \subfloat[swaptions.] {
        \includegraphics[trim=0 0 0 0,clip,width=.2\linewidth]{graphs/current_swaptions.pdf}
        \label{fig:swaptions_comp} 
      }
      \hspace*{-.1in} 
      \subfloat[vips.] {
        \includegraphics[trim=0 0 0 0,clip,width=.2\linewidth]{graphs/current_vips.pdf}
        \label{fig:vips_comp} 
      }
      \hspace*{-.1in} 
      \subfloat[water\_nsquared.] {
        \includegraphics[trim=0 0 0 0,clip,width=.2\linewidth]{graphs/current_water_nsquared.pdf}
...
\subsubsection{Decomposing the On-chip Voltage Drop}
...
\centering
   \subfloat[] {
    \includegraphics[trim=0 0 20 0,clip, height=1.25in]{graphs/pwr_loadline.pdf}
    \label{fig:pwr-loadline} 
  }
  \subfloat[] {
    \includegraphics[trim=0 0 -50 0,clip,height=1.25in]{graphs/loadline_vdd.pdf}
    \label{fig:line-vdd} 
  }
  \subfloat[] {
    \includegraphics[trim=0 0 20 0,clip,height=1.25in]{graphs/vdd_saving.pdf}
    \label{fig:vdd-saving} 
  }
  \subfloat[] {
    \includegraphics[trim=0 0 20 0,clip,height=1.25in]{graphs/loadline_freq.pdf}
    \label{fig:line-freq} 
  }
...
\subfloat[Workload consolidation.] {
    \includegraphics[trim=0 0 0 0,clip,width=.43\linewidth]{graphs/loadline-borrowing_diagram1.pdf}
    \label{fig:ll-borrow-idea1} 
}
\hfill
\subfloat[Loadline borrowing.] {
    \includegraphics[trim=0 0 0 0,clip,width=.43\linewidth]{graphs/loadline-borrowing_diagram2.pdf}
...
\subfloat[Undervolt scaling.] {
    \includegraphics[trim=0 0 65 0,clip,width=.43\linewidth]{graphs/raytrace_split_uv.pdf}
    \label{fig:split_raytrace_uv} 
}
\hfill
\subfloat[Power scaling.] {
    \includegraphics[trim=0 0 65 0,clip,width=.43\linewidth]{graphs/raytrace_split_pwr.pdf}
...
\Fig{fig:coremark-freq-vary} illustrates the problem of runtime frequency variation based on measured data. Assume critical application \benchmark{coremark} is guaranteed application performance at 4.5~GHz as part of the SLA.\footnote{We use \benchmark{coremark} because its footprint is core-contained, so it isolates interference from the memory subsystem and shows frequency changes due only to active timing margin.} This SLA can be met when the active timing margin processor is filled only with \benchmark{coremark} threads (i.e., bar in the center). However, the SLA can be violated if the scheduler coschedules \benchmark{lu\_cb} threads onto the same chip. \benchmark{coremark}'s frequency will decrease noticeably when more \benchmark{lu\_cb} threads are colocated. When only one \benchmark{coremark} is scheduled with seven other \benchmark{lu\_cb} threads (i.e., \texttt{<1,7>} on the $x$-axis), peak frequency drops to 4433~MHz from 4517~MHz. On the contrary, colocating \benchmark{mcf} leads to frequency increase. The frequency difference between coscheduling \benchmark{lu\_cb} threads and \benchmark{mcf} threads with \benchmark{coremark} is more than 100~MHz. Several other experiments across a wide variety of mappings reveal the same trend.
...
\paragraph{Solution to Guarantee Performance} 

To guarantee application QoS in the face of the active timing margin processor's variable performance, we propose \emph{adaptive mapping}, which prevents malicious co-runners from taking out the critical workload-frequency resource. \Fig{fig:adaptive-mapping-scheduler} shows our adaptive mapping's end-to-end scheduling logic. Its overall design is based on a standard feedback-driven optimization model. During every scheduling interval, the scheduler checks whether an application has high priority and whether its QoS has been violated by indexing into its job description file. If so, and if the application is sensitive to frequency, the scheduler finds the desired frequency level with the help of an application-specific frequency-QoS model. Then the scheduler locates a set of suitable co-runners that satisfy the constraint using a frequency predictor. A selected co-runner will replace the current malicious workload. This process repeats every scheduling quantum.

Because a scheduler's overall structure is fairly typical, we focus here on the components that we develop to enable adaptive mapping. These two critical components are shaded in \Fig{fig:adaptive-mapping-scheduler}. The first critical component of adaptive mapping is the frequency prediction module. It enables the scheduler to find suitable co-runners that satisfy a particular frequency target under different (hypothetical) application combinations. The second critical component is the scheduling act itself.

We present a simple MIPS-based linear model that can predict frequency accurately and quickly. Speed is of the essence because the scheduler is exploring the workload-combination space during runtime, every quantum. We construct a MIPS-based frequency prediction model because processor power consumption corresponds to active timing margin's behavior strongly (\Fig{fig:ldir-proof}), and to a first order MIPS can be used to accurately predict power. Moreover, it can be readily deployed using existing hardware performance counters.

To construct a MIPS-based prediction model, we measure active timing margin's frequency choice when all the cores are stressed by SPEC CPU 2006, PARSEC and SPLASH-2 workloads. \Fig{fig:mips-predict} shows the results. Chip total MIPS is the aggregated result of accumulating each core's individual MIPS using hardware counters. Each data point represents one benchmark; together a linear model has root mean square error of only 0.3\%. The simplicity of this model makes it a good choice for a scheduler.

\paragraph{Evaluation of Guaranteed Performance}

We demonstrate how adaptive mapping helps guarantee workload QoS using \benchmark{WebSearch}~\cite{ferdman2012clearing}, a canonical datacenter application. In our simulated scenario, \benchmark{WebSearch} runs on one core and is faced with three potential co-runners, each with a different power-consumption profile: \emph{light}, \emph{medium} and \emph{heavy}. We construct the co-runners from \benchmark{coremark} threads by constraining the issue rate of the other seven cores on which \benchmark{coremark} is running. 
Moreover, \Fig{fig:coremark-freq-vary} already shows that real workloads have a detrimental impact on clock frequency. The \emph{light}, \emph{medium} and \emph{heavy} co-runners have a MIPS of about 13,000, 28,000 and 70,000, respectively. These values are chosen because the SPEC, PARSEC and SPLASH-2 applications that we study fall into one of those three performance levels, respectively. 

The adaptive mapping scheduler aims to control \benchmark{WebSearch}'s throughput to a level that ensures that its 90$^{th}$ percentile latency meets the 0.5-second target 100\% of time when it runs by itself, i.e. with no co-runner at all. Initially, \benchmark{WebSearch} is blindly co-located with the \emph{heavy} co-runner. As times go on, the scheduler finds that QoS violates more than 25\% of the time, as shown in~\Fig{fig:tail-cdf}. Guided by the frequency predictor, and to guarantee QoS, the scheduler replaces the current co-runner with the one that has lowest MIPS, i.e., \emph{light}. This reduces the QoS violation rate to less than 7\%. As a comparison, co-locating with \emph{medium} reduces the QoS violation rate to about 15\%, which is also better than \emph{heavy}.
